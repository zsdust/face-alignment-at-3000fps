...
void getSimilarityTransform(const cv::Mat_<double>& shape_to,
	...
	for (int i = 0; i < shape_to.rows; i++){  // 生成新的shape，新的shape平均值为0
	...
	double den = 0.0;
	double temp_a = 0.0, temp_b = 0.0;
	for(int i = 0; i < shape_to.rows; ++i) {
		den += (temp2(i,0) * temp2(i, 0) + temp2(i, 1) * temp2(i, 1));
		temp_a += temp2(i, 0) * temp1(i, 0) + temp1(i, 1) * temp2(i, 1);
		temp_b += temp2(i, 0) * temp1(i, 1) - temp2(i, 1) * temp1(i, 0);
	}
	temp_a /= den;
	temp_b /= den;
	scale = sqrt(temp_a*temp_a + temp_b*temp_b);
	rotation(0, 0) = temp_a / scale;
	rotation(0, 1) = -temp_b / scale;
...
void LoadImages(std::vector<cv::Mat_<uchar> >& images,
    ...
    for (int i = 0; i < image_path_prefix.size(); i++) {
        ...
        while (fin >> image_file_name >> image_pts_name){
            ...
            cv::Mat_<uchar> image = cv::imread(image_path.c_str(), 0); // 以灰度图的形式读取
            ...
            std::vector<cv::Rect> faces; // 存放人脸所在的矩形区域
            haar_cascade.detectMultiScale(image, faces, 1.1, 2, 0, cv::Size(30, 30));  // 检测人脸
...
void LoadImages(std::vector<cv::Mat_<uchar> >& images,
    ...
    for (int i = 0; i < image_path_prefix.size(); i++) {
        ...
        while (fin >> image_file_name){
            ...
            count++;
            c++;
            cv::Mat_<uchar> figure = image.clone();
            cv::rectangle(figure, faceRec, cv::Scalar(255, 255, 255));
            cv::imshow("show image", figure);
            cv::waitKey(0);
            if (count%100 == 0){
                std::cout << count << " images loaded\n";
            }
...
double CalculateError(cv::Mat_<double>& ground_truth_shape, cv::Mat_<double>& predicted_shape){
    cv::Mat_<double> temp;
    temp = ground_truth_shape.rowRange(36, 41)-ground_truth_shape.rowRange(42, 47); // right eyes minus left eyes
    double x =mean(temp.col(0))[0];
    double y = mean(temp.col(1))[0];
    double interocular_distance = sqrt(x*x+y*y);  // this is the pupil distance
    double sum = 0;
    for (int i=0;i<ground_truth_shape.rows;i++){
        sum += norm(ground_truth_shape.row(i)-predicted_shape.row(i));
    }
    return sum/(ground_truth_shape.rows*interocular_distance); // use the pupil distance to normalize the Error to gain the geometrical invariance
}
